...
df = pd.read_csv('/Users/weiranlyu/Desktop/DecisionTree/bank/train.csv', dtype=str)
...
df['age'] = (df['age'] >= medAge).astype(int)

medBalance = df['balance'].median()
df['balance'] = (df['balance'] >= medBalance).astype(int)

medDay = df['day'].median()
df['day'] = (df['day'] >= medDay).astype(int)

medDuration = df['duration'].median()
df['duration'] = (df['duration'] >= medDuration).astype(int)

medCampaign = df['campaign'].median()
df['campaign'] = (df['campaign'] >= medCampaign).astype(int)

medPdays = df['pdays'].median()
df['pdays'] = (df['pdays'] >= medPdays).astype(int)

medPrevious = df['previous'].median()
df['previous'] = (df['previous'] >= medPrevious).astype(int)


max_depth = 1
attribute_selection = 0
ID3(df, list(df.columns), df.columns[len(df.columns) - 1], attribute_selection, max_depth)
